...
function createApiFn(config) {
  if (!isObject(config)) {
    // 非生产环境报错处理
    if (process.env.NODE_ENV !== 'production') {throw new Error(`参数${config}应该是一个对象`)}
    // 生产环境返回空对象
    return {}
  }
  const apiResult = {}
  Object.entries(config).forEach(([key, value]) => {
    ...
    if (typeof value === 'string') {
      apiResult[key] = async(data = {}) => {
        if (!isObject(config)) {
          // 非生产环境报错处理
          if (process.env.NODE_ENV !== 'production') {throw new Error(`参数${data}应该是一个对象`)}
          // 生产环境重置为空对象
          data = {}
        }
        const axiosOptions = {
      ...
      apiResult[key] = async(data = {}) => {
        ...
        if (
          ...
          // 非生产环境报错处理
          if (process.env.NODE_ENV !== 'production') {throw new Error(`参数${data}应该是一个对象或者表单对象`)}
          // 生产环境重置为空对象
          data = {}
        }
        const axiosOptions = {}
      ...
      // 非生产环境报错处理
      if (process.env.NODE_ENV !== 'production') {throw new Error(`接口配置信息${value}应该是一个url字符串或者对象`)}
    }
  })
  return apiResult
}
...
export default function(config) {
  const apiResult = createApiFn(config)
